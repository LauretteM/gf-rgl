resource ResZul = open Prelude,Predef,ParamX in {
  ...
  oper
    ...
    \vform,longform,rinit,aspect -> case <vform,longform,rinit,aspect> of {
      <VFIndic MainCl Pos PresTense,True,PhonC,(Null|Excl)> => "ya[LongPres]"++BIND ;
    ...
    verb_prefix_no_oc : VForm -> Bool -> PhonInit -> Agr -> Aspect -> Syl -> Str = \vform,longform,rinit,agr,aspect,syl -> let
      sc = case <vform,longform,rinit,aspect> of {
        ...
        <VFIndic MainCl Pos PresTense,_,PhonC,Prog> => subjConcLookup!agr!SC ++BIND++ PROG_SA ++BIND ; -- usahamba
        <VFIndic MainCl Pos PresTense,_,_,Prog> => subjConcLookup!agr!SC ++BIND++ PROG_S ++BIND ; -- useqa
        
        <VFIndic MainCl Neg PresTense,_,PhonC,Prog> => (negPref vform) ++ subjConcLookup!agr!SCNeg ++BIND++ PROG_SA ++BIND ; -- akasahambi
        <VFIndic MainCl Neg PresTense,_,_,Prog> => (negPref vform) ++ subjConcLookup!agr!SCNeg ++BIND++ PROG_S ++BIND ; -- akaseqi

        <VFIndic MainCl Pos PastTense,_,PhonC,Prog> => subjConcLookup!agr!SC ++BIND++ PROG_SA ++BIND ; -- usahambile
        <VFIndic MainCl Pos PastTense,_,_,Prog> => subjConcLookup!agr!SC ++BIND++ PROG_S ++BIND ; -- useqile

        <VFIndic MainCl Neg PastTense,_,PhonC,Prog> => (negPref vform) ++ subjConcLookup!agr!SCNeg ++BIND++ PROG_SA ++BIND ; -- akasahambile
        <VFIndic MainCl Neg PastTense,_,_,Prog> => (negPref vform) ++ subjConcLookup!agr!SCNeg ++BIND++ PROG_S ++BIND ; -- akaseqile

        <VFIndic MainCl Pos RemPastTense,_,PhonC,Prog> => (subjConc vform agr True rinit) ++ (tensePref vform PhonC syl) ++ PROG_SA ++BIND ; -- wasahamba
        <VFIndic MainCl Pos RemPastTense,_,_,Prog> => (subjConc vform agr True rinit) ++ (tensePref vform PhonC syl) ++ PROG_S ++BIND ; -- waseqa

        <VFIndic MainCl Neg RemPastTense,_,PhonC,Prog> => (negPref vform) ++ subjConcLookup!agr!SCNeg ++BIND++ PROG_SA ++BIND ; -- akasahambanga
        ...
        <VFIndic RelCl Pos RemPastTense,_,PhonC,Prog> => (relConc vform agr rinit) ++ (tensePref vform rinit syl) ++ PROG_SA ++BIND ;
        ...
        <VFIndic RelCl _ _,_,PhonC,Prog> => (relConc vform agr rinit) ++ PROG_SA ++BIND ;
        ...
        <VFIndic MainCl Neg PresTense,_,PhonC,Excl> => (negPref vform) ++ subjConcLookup!agr!SCNeg ++BIND++ EXCL_KA ++ BIND ; -- akakahambi
        ...
        <VFIndic MainCl Neg PastTense,_,PhonC,Excl> => nonExist ;
        <VFIndic MainCl Neg PastTense,_,_,Excl> => nonExist ;

        <VFIndic MainCl Pos RemPastTense,_,PhonC,Excl> => EXCL_SE ++BIND++ (subjConc vform agr True rinit) ++ (tensePref vform rinit syl) ; -- sewahamba
        <VFIndic MainCl Pos RemPastTense,_,_,Excl> => EXCL_SE ++BIND++ (subjConc vform agr True rinit) ++ (tensePref vform rinit syl) ; -- seweqa

        <VFIndic MainCl Neg RemPastTense,_,PhonC,Excl> => nonExist ;
        ...
        <VFIndic RelCl Neg PresTense,_,PhonC,Excl> => (relConc vform agr rinit) ++ EXCL_KA ++ BIND ; -- engingakahambi
        <VFIndic RelCl Neg PresTense,_,_,Excl> => (relConc vform agr PhonC) ++ EXCL_K ++ BIND ; -- engingakeqi

        <VFIndic RelCl Pos PastTense,_,PhonC,Excl> => shortRelConc!agr ++ EXCL_SE ++BIND++ subjConcLookup!agr!SC ++BIND ; -- esengihambile
        <VFIndic RelCl Pos PastTense,_,_,Excl> => shortRelConc!agr ++ EXCL_SE ++BIND++ (subjConc vform agr True rinit) ; -- esengeqile

        <VFIndic RelCl Neg PastTense,_,PhonC,Excl> => nonExist ;
        <VFIndic RelCl Neg PastTense,_,_,Excl> => nonExist ;

        <VFIndic RelCl Pos RemPastTense,_,PhonC,Excl> => shortRelConc!agr ++ EXCL_SE ++BIND++ (subjConc vform agr True rinit) ++ (tensePref vform rinit syl) ; -- osewahamba
        <VFIndic RelCl Pos RemPastTense,_,_,Excl> => shortRelConc!agr ++ EXCL_SE ++BIND++ (subjConc vform agr True rinit) ; -- oseweqa

        <VFIndic RelCl Neg RemPastTense,_,PhonC,Excl> => nonExist ;
    ...
    verb_prefix_with_oc : VForm -> Bool -> PhonInit -> Agr -> Aspect -> Syl -> Str = \vform,longform,rinit,agr,aspect,syl -> let
      sc = case <vform,longform,aspect> of {
        ...
        <VFIndic MainCl Pos RemPastTense,_,Prog> => (subjConc vform agr True rinit) ++ (tensePref vform PhonC syl) ++ PROG_SA ++BIND ; -- wasahamba
    ...
    --     <VFIndic MainCl Pos RemPastTense,_> => (negPref vform) ++ (subjConc vform agr True RA)++BIND++ (tensePref vform PhonC SylMult) ;
    ...
    tensePref : VForm -> PhonInit -> Syl -> Str = \vform,r,syl ->
      case <r,vform,syl> of {
        <PhonC,VFIndic _ Pos FutTense,SylMono> => TEMP_PREF_FUT++BIND++"ku[FutExt]" ++BIND ;
        <PhonC,VFIndic _ Pos FutTense,_> => TEMP_PREF_FUT ++BIND ;
        <_,VFIndic _ Pos FutTense,_> => TEMP_PREF_FUT++BIND++"kw[FutExt]" ++BIND ;
        <PhonC,VFIndic _ Neg FutTense,_> => TEMP_PREF_FUT_NEG++BIND++"ku[FutExt]" ++BIND ;
        <_,VFIndic _ Neg FutTense,_> => TEMP_PREF_FUT_NEG++BIND++"kw[FutExt]" ++BIND ;

        <PhonC,VFIndic _ Pos RemFutTense,SylMono> => TEMP_PREF_REMFUT++BIND++"ku[FutExt]" ++BIND ;
        ...
        <PhonC,VFIndic _ Neg RemFutTense,_> => TEMP_PREF_REMFUT_NEG++BIND++"ku[FutExt]" ++BIND ;
    ...
    relConcLookup : Agr => PhonInit => Str =
      table {
        Third C1_2 Sg => table { PhonO => "[RC][1]"++BIND ; (PhonA|PhonE) => "ow[RC][1]"++BIND ; _ => "o[RC][1]"++BIND } ;
        Third C1_2 Pl => table { PhonC => "aba[RC][2]"++BIND ; _ => "ab[RC][2]"++BIND } ;
        Third C1a_2a Sg => table { PhonO => "[RC][1a]"++BIND ; (PhonA|PhonE) => "ow[RC][1a]"++BIND ; _ => "o[RC][1a]"++BIND } ;
        Third C1a_2a Pl => table { PhonC => "aba[RC][2a]"++BIND ; _ => "ab[RC][2a]"++BIND } ;
        Third C3_4 Sg  => table { PhonO => "[RC][3]"++BIND ; (PhonA|PhonE) => "ow[RC][3]"++BIND ; _ => "o[RC][3]"++BIND } ;
        Third C3_4 Pl => table { PhonE => "[RC][4]"++BIND ; (PhonA|PhonO) => "ey[RC][4]" ++BIND ; _ => "e[RC][4]"++BIND } ;
        Third C5_6 Sg => table { PhonC => "eli[RC][5]"++BIND ; _ => "el[RC][5]"++BIND } ;
        Third C5_6 Pl => table { PhonC => "a[RC][6]"++BIND ; _ => "[RC][6]"++BIND } ;
        Third C7_8 Sg => table { PhonC => "esi[RC][7]"++BIND ; _ => "es[RC][7]"++BIND } ;
        Third C7_8 Pl => table { PhonC => "ezi[RC][8]"++BIND ; _ => "ez[RC][8]"++BIND } ;
        Third C9_10 Sg => table { PhonE => "[RC][9]"++BIND ; (PhonA|PhonO) => "ey[RC][9]" ++BIND ; _ => "e[RC][9]"++BIND } ;
        Third C9_10 Pl => table { PhonC => "ezi[RC][10]"++BIND ; _ => "ez[RC][10]"++BIND } ;
        Third C11_10 Sg => table { PhonC => "olu[RC][11]"++BIND ; (PhonA|PhonE) => "olw[RC][11]" ; _ => "ol[RC][11]"++BIND } ;
        Third C11_10 Pl => table { PhonC => "ezi[RC][10]"++BIND ; _ => "ez[RC][10]"++BIND } ;
        Third C9_6 Sg => table { PhonE => "[RC][9]"++BIND ; (PhonA|PhonO) => "ey[RC][9]" ++BIND ; _ => "e[RC][9]"++BIND } ;
        Third C9_6 Pl => table { PhonC => "a[RC][6]"++BIND ; _ => "[RC][6]"++BIND } ;
        Third C14 _ => table { PhonC => "obu[RC][14]"++BIND ; _ => "ob[RC][14]"++BIND } ;
        Third C15 _ => table { PhonC => "oku[RC][15]"++BIND ; (PhonA|PhonE) => "okw[RC][15]" ; _ => "ok[RC][15]"++BIND } ;
        Third C17 _ => table { PhonC => "oku[RC][17]"++BIND ; (PhonA|PhonE) => "okw[RC][17]" ; _ => "ok[RC][17]"++BIND } ;
        First Sg => table { PhonC => "engi[RC][1ps]"++BIND ; _ => "eng[RC][1ps]"++BIND } ;
        First Pl => table { PhonC => "esi[RC][1pp]"++BIND ; _ => "es[RC][1pp]"++BIND } ;
        Second Sg  => table { PhonE => "ow[RC][2ps]"++BIND ; _ => "o[RC][2ps]"++BIND } ;
        Second Pl => table { PhonC => "eni[RC][2pp]"++BIND ; _ => "en[RC][2pp]"++BIND }
    } ;

    adjConcLookup : Agr => PhonInit => Str =
      table {
        Third C1_2 Sg => table { PhonO => "[AC][1]" ; (PhonA|PhonE) => "ow[AC][1]"++BIND ; _ => "o[AC][1]"++BIND } ;
        Third C1_2 Pl => table { PhonC => "aba[AC][2]"++BIND ; _ => "ab[AC][2]"++BIND } ;
        Third C1a_2a Sg => table { PhonO => "[AC][1a]" ; (PhonA|PhonE) => "ow[AC][1a]"++BIND ; _ => "o[AC][1a]"++BIND } ;
        Third C1a_2a Pl => table { PhonC => "aba[AC][2a]"++BIND ; _ => "ab[AC][2a]"++BIND } ;
        Third C3_4 Sg  => table { PhonO => "[AC][3]" ; (PhonA|PhonE) => "ow[AC][3]"++BIND ; _ => "o[AC][3]"++BIND } ;
        Third C3_4 Pl => table { PhonC => "emi[AC][4]"++BIND ; _ => "em[AC][4]"++BIND } ;
        Third C5_6 Sg => table { PhonC => "eli[AC][5]"++BIND ; _ => "el[AC][5]"++BIND } ;
        Third C5_6 Pl => table { PhonC => "a[AC][6]"++BIND ; _ => "[AC][6]" } ;
        Third C7_8 Sg => table { PhonC => "esi[AC][7]"++BIND ; _ => "es[AC][7]"++BIND } ;
        Third C7_8 Pl => table { PhonC => "ezi[AC][8]"++BIND ; _ => "ez[AC][8]"++BIND } ;
        Third C9_10 Sg => table { PhonE => "[AC][9]" ; (PhonA|PhonO) => "ey[AC][9]"++BIND ; _ => "e[AC][9]"++BIND } ;
        Third C9_10 Pl => table { PhonC => "ezi[AC][10]"++BIND ; _ => "ez[AC][10]"++BIND } ;
        Third C11_10 Sg => table { PhonC => "olu[AC][11]"++BIND ; (PhonA|PhonE) => "olw[AC][11]"++BIND ; _ => "ol[AC][11]"++BIND } ;
        Third C11_10 Pl => table { PhonC => "ezi[AC][10]"++BIND ; _ => "ez[AC][10]"++BIND } ;
        Third C9_6 Sg => table { PhonE => "[AC][9]" ; (PhonA|PhonO) => "ey[AC][9]"++BIND ; _ => "e[AC][9]"++BIND } ;
        Third C9_6 Pl => table { PhonC => "a[AC][6]"++BIND ; _ => "[AC][6]" } ;
        Third C14 _ => table { PhonC => "obu[AC][14]"++BIND ; _ => "ob[AC][14]"++BIND } ;
        Third C15 _ => table { PhonC => "oku[AC][15]"++BIND ; (PhonA|PhonE) => "okw[AC][15]" ; _ => "ok[AC][15]"++BIND } ;
        Third C17 _ => table { PhonC => "oku[AC][17]"++BIND ; (PhonA|PhonE) => "okw[AC][17]" ; _ => "ok[AC][17]"++BIND } ;
        First Sg => table { PhonC => "engi[AC][1ps]"++BIND ; _ => "eng[AC][1ps]"++BIND } ;
        First Pl => table { PhonC => "esi[AC][1pp]"++BIND ; _ => "es[AC][1pp]"++BIND } ;
        Second Sg  => table { PhonE => "ow[AC][2ps]"++BIND ; _ => "o[AC][2ps]"++BIND } ;
        Second Pl => table { PhonC => "eni[AC][2pp]"++BIND ; _ => "en[AC][2pp]"++BIND }
    } ;
    ...
    poss_concord_agr : Agr => PhonInit => Str =
      table {
        First Sg => table {(PhonA|PhonC) => "wa[PC][1ps]" ; (PhonE|PhonI) => "we[PC][1ps]" ; (PhonO|PhonU) => "wo[PC][1ps]" } ;
        First Pl => table {(PhonA|PhonC) => "ba[PC][1pp]" ; (PhonE|PhonI) => "be[PC][1pp]" ; (PhonO|PhonU) => "bo[PC][1pp]" } ;
        Second Sg => table {(PhonA|PhonC) => "wa[PC][2ps]" ; (PhonE|PhonI) => "we[PC][2ps]" ; (PhonO|PhonU) => "wo[PC][2ps]" } ;
        Second Pl => table {(PhonA|PhonC) => "ba[PC][2pp]" ; (PhonE|PhonI) => "be[PC][2pp]" ; (PhonO|PhonU) => "bo[PC][2pp]" } ;
        Third C1_2 Sg => table {(PhonA|PhonC) => "wa[PC][1]" ; (PhonE|PhonI) => "we[PC][1]" ; (PhonO|PhonU) => "wo[PC][1]" } ;
        Third C1_2 Pl => table {(PhonA|PhonC) => "ba[PC][2]" ; (PhonE|PhonI) => "be[PC][2]" ; (PhonO|PhonU) => "bo[PC][2]" } ;
        Third C1a_2a Sg => table {(PhonA|PhonC) => "wa[PC][1a]" ; (PhonE|PhonI) => "we[PC][1a]" ; (PhonO|PhonU) => "wo[PC][1a]" } ;
        Third C1a_2a Pl => table {(PhonA|PhonC) => "ba[PC][2a]" ; (PhonE|PhonI) => "be[PC][2a]" ; (PhonO|PhonU) => "bo[PC][2a]" } ;
        Third C3_4 Sg => table {(PhonA|PhonC) => "wa[PC][3]" ; (PhonE|PhonI) => "we[PC][3]" ; (PhonO|PhonU) => "wo[PC][3]" } ;
        Third C3_4 Pl => table {(PhonA|PhonC) => "ya[PC][4]" ; (PhonE|PhonI) => "ye[PC][4]" ; (PhonO|PhonU) => "yo[PC][4]" } ;
        Third C5_6 Sg => table {(PhonA|PhonC) => "la[PC][5]" ; (PhonE|PhonI) => "le[PC][5]" ; (PhonO|PhonU) => "lo[PC][5]" } ;
        Third C5_6 Pl => table {(PhonA|PhonC) => "a[PC][6]" ; (PhonE|PhonI) => "e[PC][6]" ; (PhonO|PhonU) => "o[PC][6]" } ;
        Third C7_8 Sg => table {(PhonA|PhonC) => "sa[PC][7]" ; (PhonE|PhonI) => "se[PC][7]" ; (PhonO|PhonU) => "so[PC][7]" } ;
        Third C7_8 Pl => table {(PhonA|PhonC) => "za[PC][8]" ; (PhonE|PhonI) => "ze[PC][8]" ; (PhonO|PhonU) => "zo[PC][8]" } ;
        Third C9_10 Sg => table {(PhonA|PhonC) => "ya[PC][9]" ; (PhonE|PhonI) => "ye[PC][9]" ; (PhonO|PhonU) => "yo[PC][9]" } ;
        Third C9_10 Pl => table {(PhonA|PhonC) => "za[PC][10]" ; (PhonE|PhonI) => "ze[PC][10]" ; (PhonO|PhonU) => "zo[PC][10]" } ;
        Third C11_10 Sg => table {(PhonA|PhonC) => "lwa[PC][11]" ; (PhonE|PhonI) => "lwe[PC][11]" ; (PhonO|PhonU) => "lo[PC][11]" } ;
        Third C11_10 Pl => table {(PhonA|PhonC) => "za[PC][10]" ; (PhonE|PhonI) => "ze[PC][10]" ; (PhonO|PhonU) => "zo[PC][10]" } ;
        Third C9_6 Sg => table {(PhonA|PhonC) => "ya[PC][9]" ; (PhonE|PhonI) => "ye[PC][9]" ; (PhonO|PhonU) => "yo[PC][9]" } ;
        Third C9_6 Pl => table {(PhonA|PhonC) => "a[PC][6]" ; (PhonE|PhonI) => "e[PC][6]" ; (PhonO|PhonU) => "o[PC][6]" } ;
        Third C14 _ => table {(PhonA|PhonC) => "ba[PC][14]" ; (PhonE|PhonI) => "be[PC][14]" ; (PhonO|PhonU) => "bo[PC][14]" } ;
        Third C15 _ => table {(PhonA|PhonC) => "kwa[PC][15]" ; (PhonE|PhonI) => "kwe[PC][15]" ; (PhonO|PhonU) => "ko[PC][15]" } ;
        Third C17 _ => table {(PhonA|PhonC) => "kwa[PC][17]" ; (PhonE|PhonI) => "kwe[PC][17]" ; (PhonO|PhonU) => "ko[PC][17]" }
      } ;

    poss_concord : ClassGender => Number => PhonInit => Str =
      table {
        C1_2 => table {
          Sg => table {(PhonA|PhonC) => "wa[PC][1]" ; (PhonE|PhonI) => "we[PC][1]" ; (PhonO|PhonU) => "wo[PC][1]" } ;
          Pl => table {(PhonA|PhonC) => "ba[PC][2]" ; (PhonE|PhonI) => "be[PC][2]" ; (PhonO|PhonU) => "bo[PC][2]" }
        } ;
        C1a_2a => table {
          Sg => table {(PhonA|PhonC) => "wa[PC][1a]" ; (PhonE|PhonI) => "we[PC][1a]" ; (PhonO|PhonU) => "wo[PC][1a]" } ;
          Pl => table {(PhonA|PhonC) => "ba[PC][2a]" ; (PhonE|PhonI) => "be[PC][2a]" ; (PhonO|PhonU) => "bo[PC][2a]" }
        } ;
        C3_4 => table {
          Sg => table {(PhonA|PhonC) => "wa[PC][3]" ; (PhonE|PhonI) => "we[PC][3]" ; (PhonO|PhonU) => "wo[PC][3]" } ;
          Pl => table {(PhonA|PhonC) => "ya[PC][4]" ; (PhonE|PhonI) => "ye[PC][4]" ; (PhonO|PhonU) => "yo[PC][4]" }
        } ;
        C5_6 => table {
          Sg => table {(PhonA|PhonC) => "la[PC][5]" ; (PhonE|PhonI) => "le[PC][5]" ; (PhonO|PhonU) => "lo[PC][5]" } ;
          Pl => table {(PhonA|PhonC) => "a[PC][6]" ; (PhonE|PhonI) => "e[PC][6]" ; (PhonO|PhonU) => "o[PC][6]" }
        } ;
        C7_8 => table {
          Sg => table {(PhonA|PhonC) => "sa[PC][7]" ; (PhonE|PhonI) => "se[PC][7]" ; (PhonO|PhonU) => "so[PC][7]" } ;
          Pl => table {(PhonA|PhonC) => "za[PC][8]" ; (PhonE|PhonI) => "ze[PC][8]" ; (PhonO|PhonU) => "zo[PC][8]" }
        } ;
        C9_10 => table {
          Sg => table {(PhonA|PhonC) => "ya[PC][9]" ; (PhonE|PhonI) => "ye[PC][9]" ; (PhonO|PhonU) => "yo[PC][9]" } ;
          Pl => table {(PhonA|PhonC) => "za[PC][10]" ; (PhonE|PhonI) => "ze[PC][10]" ; (PhonO|PhonU) => "zo[PC][10]" }
        } ;
        C11_10 => table {
          Sg => table {(PhonA|PhonC) => "lwa[PC][11]" ; (PhonE|PhonI) => "lwe[PC][11]" ; (PhonO|PhonU) => "lo[PC][11]" } ;
          Pl => table {(PhonA|PhonC) => "za[PC][10]" ; (PhonE|PhonI) => "ze[PC][10]" ; (PhonO|PhonU) => "zo[PC][10]" }
        } ;
        C9_6 => table {
          Sg => table {(PhonA|PhonC) => "ya[PC][9]" ; (PhonE|PhonI) => "ye[PC][9]" ; (PhonO|PhonU) => "yo[PC][9]" } ;
        ...
        C14 => table {
          _ => table {(PhonA|PhonC) => "ba[PC][14]" ; (PhonE|PhonI) => "be[PC][14]" ; (PhonO|PhonU) => "bo[PC][14]" }
        } ;
        C15 => table {
          _ => table {(PhonA|PhonC) => "kwa[PC][15]" ; (PhonE|PhonI) => "kwe[PC][15]" ; (PhonO|PhonU) => "ko[PC][15]" }
        } ;
        C17 => table {
          _ => table {(PhonA|PhonC) => "kwa[PC][17]" ; (PhonE|PhonI) => "kwe[PC][17]" ; (PhonO|PhonU) => "ko[PC][17]" }
        }
      } ;
    ...
    in
      case vform of {
        ...
        VFIndic _ _ FutTense => sc ++ (tensePref vform PhonC SylMult) ++ "ba[AuxV]" ;
        VFIndic _ _ RemFutTense => sc ++ (tensePref vform PhonC SylMult) ++ "ba[AuxV]" ;
        VFIndic _ _ _ => [] ;
        VFConsec _ => [] ;
        VFSubjunct _ => []
    } ;
    ...
    -- for positive, present: SC only inserted with class 9
      case vform of {
        ...
        VFIndic MainCl Pos (FutTense|RemFutTense) => case agr of {
          ...
          Third _ _ | First _ | Second _ => sc ++ (tensePref vform PhonC SylMult) ++ "ba[AuxV]" -- ++BIND
        } ;
        VFIndic RelCl Pos (FutTense|RemFutTense) => case agr of {
          -- Third C9_10 Sg | Third C9_6 Sg => (tensePref vform PhonC SylMult) ++ "b" ; -- ++BIND ;
          Third _ _ | First _ | Second _ => (tensePref vform PhonC SylMult) ++ "ba[AuxV]" -- ++BIND
        } ;
        VFIndic MainCl Neg (FutTense|RemFutTense) => case agr of {
          ...
          Third _ _ | First _ | Second _ => "a[NegPre]" ++BIND++ sc ++ (tensePref vform PhonC SylMult) ++ "ba[AuxV]"
        } ;
        VFIndic RelCl Neg (FutTense|RemFutTense) => (negPrefNge vform False) ++ (tensePref vform PhonC SylMult) ++ "ba[AuxV]" ;
      ...
      in case vform of {
        ...
        VFConsec Pos => (subjConc vform agr False PhonC) ++ "ba[AuxV]" ;
        VFConsec Neg => (subjConc vform agr False PhonC) ++ (negPrefNga vform False) ++ "bi[AuxV]" ;
        VFSubjunct Pos => (subjConc vform agr False PhonC) ++ "be[AuxV]" ;
        VFSubjunct Neg => (subjConc vform agr False PhonC) ++ (negPrefNga vform False) ++ "bi[AuxV]"
      } ;
      ...
      kho_cop : VForm -> Agr -> Str = \vform,agr -> case vform of {
        ...
        VFIndic RelCl Neg PresTense => (relConcCop vform agr PhonC) ++ (ap_cop_pref (VFIndic RelCl Neg PresTense) agr RelType) ++ "kho[Adv]" ;
        VFIndic RelCl p t => (relConcCop vform agr PhonC) ++ (ap_cop_pref (VFIndic RelCl p t) agr RelType) ++ "khona[Adv]" ;
    ...
    regConjN : Str -> { s : PhonInit => Str ; fix : Bool } = \s -> {
      s = \\_ => s+"[Conj]" ;
      fix = False
    } ;
