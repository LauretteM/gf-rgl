concrete VerbSBantuSsw of VerbSBantu = CatSsw,CatSBantuSsw ** open ResSsw, Prelude, ParamX in {
  ...
  lin
    ...
    CopAP ap = {
      ...
      imp_s = table {
        Sg => let
          agr = (Second Sg)
        in table {
          Pos => BA++BIND++IMP_SUF ++ ap.s!AF1 ;
          Neg => IMP_NEG_PREF_SG++BIND++BI ++ ap.s!AF1
        } ;
        Pl => let
          agr = (Second Pl)
        in table {
          Pos => BA++BIND++IMP_SUF++BIND++PL_NI ++ ap.s!AF1 ;
    ...
    CopNP np = {
      ...
      imp_s = table {
        Sg => table {
          Pos => BA++BIND++IMP_SUF ++ np.s!NFull ;
          Neg => IMP_NEG_PREF_SG++BIND++BI ++ np.s!NFull
        } ;
        Pl => table {
          Pos => BA++BIND++IMP_SUF++BIND++PL_NI ++ np.s!NFull ;
          Neg => IMP_NEG_PREF_PL++BIND++BI ++ np.s!NFull
        }
      } ;
      inf_s = table {
          NFull => table {
    ...
    CopNPAssoc np = {
      ...
      in table {
        Sg => table {
          Pos => BA++BIND++IMP_SUF ++ cop ;
          Neg => IMP_NEG_PREF_SG++BIND++BI ++ cop
        } ;
        Pl => table {
          Pos => BA++BIND++IMP_SUF++BIND++PL_NI ++ cop ;
    ...
    CopPoss np = {
      s = table {
        MainCl => \\a,p,t,s,l => let
          ...
          pcp = (id_pre_cop_pref vform a) ; -- u- / uzoba / akazukuba
          -- cp = (id_cop_pref np.agr) ; -- ng-
          cop_base = poss_concord_agr!(Third C17 Sg)!np.agr ++BIND++ np.s!NPoss -- utshani
        in
          pcp ++ cop_base ;
        RelCl => \\a,p,t,s,l => let
          vform = VFIndic RelCl p t ;
      ...
      imp_s = let
        cop = poss_concord_agr!(Third C17 Sg)!np.agr ++BIND++ np.s!NPoss
      in table {
        Sg => table {
          Pos => BA++BIND++IMP_SUF ++ cop ;
          Neg => IMP_NEG_PREF_SG++BIND++BI ++ cop
